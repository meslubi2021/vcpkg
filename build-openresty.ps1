...
Enter-VsDevShell -VsInstallPath $installPath -StartInPath $pwd

$vcpkg = "$pwd\installed\x86-windows-mixed"
...
# Build luarocks
tar xf .\openresty\zips\luarocks-3.3.1.zip --strip-components=1 -C .\buildtrees\openresty\luarocks\
...
tar xf .\openresty\zips\openresty-1.15.8.3-win32.zip --strip-components=1 -C "$openresty"
...
tar xf .\openresty\zips\Redis-x86-3.2.100.zip -C "$redis"
